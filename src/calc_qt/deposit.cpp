...
bool Deposit::Validate() {
    ...
    return amt > 0 && ok && time > 0 && ok1 && tax >= 0 && ok2 && perc > 0 && ok3 && add && dec && pay_t > 0 && ok4;
}
...
bool Deposit::AddValidate() {
    ...
    bool bl_add = (ui->Add->text().toDouble(&ok) == 0 && ok) || ui->Add->text().isEmpty();
    bool bl_add_t = (ui->Add_t->text().toInt(&ok1) == 0 && ok1) || ui->Add_t->text().isEmpty();
...
bool Deposit::DecValidate() {
    ...
    bool bl_dec = (ui->Dec->text().toDouble(&ok) == 0 && ok) || ui->Dec->text().isEmpty();
    bool bl_dec_t = (ui->Dec_t->text().toInt(&ok1) == 0 && ok1) || ui->Dec_t->text().isEmpty();
...
void Deposit::CalcRes() {
    if (Validate()) {
        ...
        dep_res res = deposit_res(sum, time, perc, tax, reinv, pay_t, add_t, add, dec_t, dec);
        if (!res.err) {

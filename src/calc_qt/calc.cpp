...
calc::calc(QWidget *parent) : QMainWindow(parent), ui(new Ui::calc) {
  ...
  connect(ui->Display, SIGNAL(returnPressed()), this, SLOT(EqualButtonPressed()));
  connect(ui->Display, SIGNAL(textChanged(QString)), this, SLOT(ResetStyleSheet()));
...
//    bool ok;
//    double res = ui->Display->text().toDouble(&ok);
